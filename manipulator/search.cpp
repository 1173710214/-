...
namespace search{
		...
		for (size_t i = 0; i < 10000 && vec.y != 0; i++)
		{
			...
			if (i % 1 == 0) {
				...
				for (int j = 1;j < gs->ShotNum;j++) {
					//get hitted then return {0,0}
					if (distance(x + input[2] + 2.375, gs->body[rank[j]][0], y, 27.6+4.88-gs->body[rank[j]][1]) < 0.30&&gs->body[rank[j]][1] != 0) {
						return 1;
					}
				}
	...
	*/
		...
		for (size_t i = 0; i < 10000 && vec.y != 0; i++)
		{
			...
			if (i % 1 == 0) {
				...
				for (int j = 0;j < gs->ShotNum;j++) {
					//get hitted then return {0,0}
					double x1 = x + input[2];
					double x2 = gs->body[rank[j]][0] - 2.375;
					double y1 = y;
	...
	std::vector<double> sendToMidStraight(std::vector<double> destination,bool strength,int num) {
		//speed, angle, offset
		std::vector<double> input = { 7,0,0 };
		...
		for (size_t j = 0; j < 20; j++)
		{
			...
			double disvx = vec.x;
			input = { input[0],input[1] ,input[2] + disx };
			if (abs(disy * 1.0 / 9) < 0.0001&&abs(disx)<0.000001) break;
		}
